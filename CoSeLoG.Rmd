...
7. Add annotations to screen captures.
...
1. The maximum number of events (77) occured on Apr 27, 2011 across cases.  
...
For the more recent cases there are a lot less events / activities occuring close to case initiation compared to the earlier cases - did the process change around July 2011 ?
...
#replay_ILP_lst <- replay.petrisim(ILP_pn, "T00", verbose=FALSE)
...
#replay_ILP_lst <- replay.petrisim(ILP_pn, "T00,T02,T04,T05", verbose=FALSE)
...
#replay_ILP_lst <- replay.petrisim(ILP_pn, "T00,T02,T03,T02,T05", verbose=FALSE)
ILP_traces_df <- orderBy(~type, rbind(ILP_traces_df,
                       cbind(data.frame(type="1B", trace="T00,T02,[T03,T02]*,T05"),
                             subset(ILP_traces_df, type == "1")[, 3:9])))
#replay_ILP_lst <- replay.petrisim(ILP_pn, "T00,T02,T03,T02,T04,T05", verbose=FALSE)
ILP_traces_df <- orderBy(~type, rbind(ILP_traces_df,
            cbind(data.frame(type="1BA", trace="T00,T02,[T03,T02]*,[T04]*,T05"),
                             subset(ILP_traces_df, type == "1")[, 3:9])))

#replay_ILP_lst <- replay.petrisim(ILP_pn, "T00,T06,T10,T11", verbose=FALSE)
ILP_traces_df <- orderBy(~type, rbind(ILP_traces_df,
            cbind(data.frame(type="2A", trace="T00,[T06]*,T10,T11"),
                             subset(ILP_traces_df, type == "2")[, 3:9])))
#replay_ILP_lst <- replay.petrisim(ILP_pn, "T00,T07-1,T10,T11", verbose=FALSE)
ILP_traces_df <- orderBy(~type, rbind(ILP_traces_df,
            cbind(data.frame(type="2B", trace="T00,[T07-1]*,T10,T11"),
                             subset(ILP_traces_df, type == "2")[, 3:9])))
#replay_ILP_lst <- replay.petrisim(ILP_pn, "T00,T07-1,T06,T10,T11", verbose=FALSE)
ILP_traces_df <- orderBy(~type, rbind(ILP_traces_df,
...
print(subset(ILP_traces_df, type == "2C1b", select=-c(Start)), row.names=FALSE)

#replay.petrisim(ILP_pn, "T00,T06,T10,T02,T03,T02,T04,T05", verbose=FALSE)
ILP_traces_df <- orderBy(~type, rbind(ILP_traces_df,
    cbind(data.frame(type="2CA1a", trace="T00,[T06]*,T10,T02,[T03,T02]*,[T04]*,T05"),
                                subset(ILP_traces_df, type == "2C1a")[, 3:9])))
#replay.petrisim(ILP_pn, "T00,T06,T10,T11,T02,T03,T02,T04,T05", verbose=FALSE)
ILP_traces_df <- orderBy(~type, rbind(ILP_traces_df,
    cbind(data.frame(type="2CA1b", trace="T00,[T06]*,T10,T11,T02,[T03,T02]*,[T04]*,T05"),
                                subset(ILP_traces_df, type == "2C1b")[, 3:9])))

#replay.petrisim(ILP_pn, "T00,T07-1,T10,T02,T03,T02,T04,T05", verbose=FALSE)
ILP_traces_df <- orderBy(~type, rbind(ILP_traces_df,
    cbind(data.frame(type="2CB1a", trace="T00,[T07-1]*,T10,T02,[T03,T02]*,[T04]*,T05"),
                                subset(ILP_traces_df, type == "2C1a")[, 3:9])))
#replay.petrisim(ILP_pn, "T00,T07-1,T10,T11,T02,T03,T02,T04,T05", verbose=FALSE)
ILP_traces_df <- orderBy(~type, rbind(ILP_traces_df,
    cbind(data.frame(type="2CB1b", trace="T00,[T07-1]*,T10,T11,T02,[T03,T02]*,[T04]*,T05"),
                                subset(ILP_traces_df, type == "2C1b")[, 3:9])))

#replay.petrisim(ILP_pn, "T00,T07-1,T06,T10,T02,T03,T02,T04,T05", verbose=FALSE)
ILP_traces_df <- orderBy(~type, rbind(ILP_traces_df,
...
```{r Describe_Selected_Petrinet_4, echo=FALSE, fig.height=5.0}
replay.petrisim(ILP_pn, "T00,T07-1,T06,T10,T11,T02,T03,T02,T04,T05", verbose=FALSE)
ILP_traces_df <- orderBy(~type, rbind(ILP_traces_df,
    cbind(data.frame(type="2CBA1b", trace="T00,[T07-1]*,[T06]*,T10,T11,T02,[T03,T02]*,[T04]*,T05"),
                                subset(ILP_traces_df, type == "2C1b")[, 3:9])))

row.names(ILP_traces_df) <- 1:nrow(ILP_traces_df)
print(subset(ILP_traces_df, select=-c(Start)))
```
...
![](CoSeLoG_Step_06_PetriNet_Normative_Conformance_Zoom.png)
...
![](CoSeLoG_Step_06_PetriNet_Normative_Conformance_T06.png)
