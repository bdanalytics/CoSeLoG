...
petrinets_df <- data.frame("Petrinet"=c("Alpha"),
                           ...
                           "Simplicity.Transitions.Silent"=c(0),
...
The ILP algorithm has discovered 9 transitions & 8 places ("P 1" & "P 2" are "End" places). Additionally, the ILP Petri net handles transitions T06, T07-1 & T10 better by not isolating them from the control-flow. Collect Petri net evaluation metrics as listed in the Appendix section "Actions to run ProM plug-in "Replay a Log on Petri Net for Conformance Analysis"".  
...
This approach has discovered 9 transitions again, 14 "hidden" / "silent" transitions and 18 places. However, there does not seem to be a clear "End" place. It appears that "pi1" is the closest "End" place.    
...
petrinets_df <- rbind(petrinets_df, 
                data.frame("Petrinet"=c("Heuristics"),
                           ...
                           "Simplicity.Places.Implicit"=c(0),
                           "Simplicity.Transitions.Silent"=c(14),
...
petrinets_df <- rbind(petrinets_df, 
                data.frame("Petrinet"=c("Inductive"),
                           ...
                           "Simplicity.Places.Implicit"=c(0),
                           "Simplicity.Transitions.Silent"=c(25),
...
To consolidate the Simplicity.* metrics into a Simplicity.Score for each Petri net, assign equal weights (as a simple approximation) by scaling the raw metric relative to a "perfect" score of 100 since each metric has different scales. For e.g. the minimum value of Simplicity.Places.Total is 8 which may be considered as the "perfect" score amongst these Petri nets. If the minimum value is 0 we add 1 to both numerator & denominator.

```{r Select_Petrinet_2, echo=TRUE, fig.height=10}
consolidate_scores <- function(pn_df, metrics, consolidated.metric.name) {
    ...
    for (row in 1:nrow(pn_df))
        pn_df[row, consolidated.metric.name] <- 
            mean(as.numeric(pn_df[row, paste0(metrics, ".Scaled")]))

#     print(myplot_hbar(pn_df, ".rownames", 
#                       c("Simplicity.Places.Total", "Simplicity.Places.Total.Scaled")))
    
    df <- pn_df
    xcol_name <- ".rownames"
    ycol_names <- c("Simplicity.Places.Total", "Simplicity.Places.Total.Scaled",
                    "Simplicity.Places.Implicit", "Simplicity.Places.Implicit.Scaled",
                    "Simplicity.Transitions.Silent", "Simplicity.Transitions.Silent.Scaled",
                    "Simplicity.Arcs", "Simplicity.Arcs.Scaled",
                    "Simplicity.MDL", "Simplicity.MDL.Scaled"
                    )
    ...
    # New
#     mltd_df <- mutate(mltd_df, 
#             var_type=ifelse(length(grep(".Scaled", variable, fixed=TRUE)) == 0,
#                             "Raw", "Score"))
    mltd_df$variable <- as.character(mltd_df$variable)
    mltd_df$var_type <- sapply(1:nrow(mltd_df), function(row)
            ifelse(length(grep(".Scaled", mltd_df[row, "variable"], fixed=TRUE)) == 0,
                            "Raw", "Scaled"))
    mltd_df$var_label <- sapply(1:nrow(mltd_df), function(row)
            ifelse(length(grep(".Scaled", mltd_df[row, "variable"], fixed=TRUE)) == 0,
                            mltd_df[row, "variable"], 
                            gsub(".Scaled", "", mltd_df[row, "variable"], fixed=TRUE)))
    #print(mltd_df)
    
    #g <- ggplot(mltd_df, aes(x=reorder(xcol_name, value), y=value, fill=variable))
    g <- ggplot(mltd_df, aes(x=reorder(xcol_name, value), y=value))        
    g <- g + geom_bar(stat="identity", aes(fill=var_label)) + xlab(xcol_name)

    # New
    g <- g + facet_grid(var_label ~ var_type, scales="free") + 
...
#     print(myplot_hbar(pn_df, ".rownames", c("Simplicity.Places.Total.Scaled",
#                                             "Simplicity.Places.Implicit.Scaled",
#                                             "Simplicity.Transitions.Silent.Scaled",
#                                             "Simplicity.Arcs.Scaled",
#                                             "Simplicity.MDL.Scaled")))

    radar_inp_df <- cbind( data.frame(instance=rownames(df)),
                                    subset(pn_df, select=c( 
                                        Simplicity.Places.Total.Scaled,
                                        Simplicity.Places.Implicit.Scaled,
                                        Simplicity.Transitions.Silent.Scaled,
                                        Simplicity.Arcs.Scaled,
                                        Simplicity.MDL.Scaled)),
    ...
    print(myplot_hbar(pn_df, ".rownames", "Simplicity.Score"))
    
    return(pn_df)
}
petrinets_mod_df <- consolidate_scores(pn_df=petrinets_df, 
        metrics=grep("Simplicity.", names(petrinets_df), fixed=TRUE, value=TRUE), 
        consolidated.metric.name="Simplicity.Score")
print(petrinets_mod_df)
```
...
'+' Integrate all event log transitions into the control-flow (ILP, Heuristics & Inductive).    
'+' No silent transitions (ILP & Alpha).  
...
**1**. Start -> T00 -> T02 -> T05 -> End01 with 2 tokens remaining in ILP3 (between T00 & T10) and ILP5 (between T00 & T07-1)

**2**. Start -> T00 -> T10 -> T11 -> End02 with 1 token remaining in ILP1 (between T00 & T02)

The traces with some loops include:  
**1A**. Start -> T00 -> T02 -> [T04]* -> T05 -> End01  
    After T02, there might be any number of T04 firings
    
**1B**. Start -> T00 -> T02 -> [T03 -> T02]* -> T05 -> End01  
    After T02, there might be any number of T03 -> T02 loops
    
**1AB**. Start -> T00 -> T02 -> [T04]* -> [T03 -> T02]* -> T05 -> End01  
    After T02, there might be any number of T04 firings and/or T03 -> T02 loops
    
**2A**. Start -> T00 -> [T06]* -> T10 -> T11 -> End02

**2B**. Start -> T00 -> [T07-1]* -> T10 -> T11 -> End02

**2AB**. Start -> T00 -> [T06]* -> [T07-1]* -> T10 -> T11 -> End02

**2C1a**. Start -> T00 -> T10 -> T02 -> [T04]* -> [T03 -> T02]* -> T05 -> End01  
**2C1b**. Start -> T00 -> T10 -> T11 -> T02 -> [T04]* -> [T03 -> T02]* -> T05 -> End01  
...
2. Simplicity metrics:  
    2.1 # total places  
    2.2 # implicit places  
    2.3 # silent transitions  
    2.4 # arcs  
    2.5 Entropy: ProM plug-in ?  
    2.6 Minimal Description Length (MDL): ProM plug-in ?
...
Based on the analysis objective(s), these metrics may be assigned appropriate weights. For this analysis, equal weights were selected i.e. 25% for each metric category and 20% for the Simplicity metric set elements (Entropy was not computed).
