...
consolidate_scores <- function(pn_df, metrics, consolidated.metric.name, 
    ...
    gp <- ggplot(hbar_df, aes(x=reorder(Petrinet, value), y=value)) +       
            ...
            facet_grid(var_label ~ var_type, scales="free", space="free") + 
...
Based on Simplicity metrics, Alpha & ILP have high scores. Let's create scores for the other categories. The "Raw" vs. "Scaled" bar graphs should have scaled x-axis but for some reason that is not happening.

```{r Select_Petrinet_3, echo=TRUE}
