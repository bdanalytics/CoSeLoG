...
consolidate_scores <- function(pn_df, metrics, consolidated.metric.name) {
    ...
    for (row in 1:nrow(pn_df))
        pn_df[row, consolidated.metric.name] <- 
            mean(as.numeric(pn_df[row, paste0(metrics, ".Score")]))

    print(myplot_hbar(pn_df, ".rownames", 
                      c("Simplicity.Places.Total", "Simplicity.Places.Total.Score")))
    
    df <- pn_df
    xcol_name <- ".rownames"
    ycol_names <- c("Simplicity.Places.Total", "Simplicity.Places.Total.Score",
                    "Simplicity.Places.Implicit", "Simplicity.Places.Implicit.Score",
                    "Simplicity.Transitions.Silent", "Simplicity.Transitions.Silent.Score",
                    "Simplicity.Arcs", "Simplicity.Arcs.Score",
                    "Simplicity.MDL", "Simplicity.MDL.Score"
                    )
