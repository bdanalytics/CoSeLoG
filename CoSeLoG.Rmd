...
```{r Select_Petrinet_2, echo=TRUE, fig.height=6.5}
...
#                     )    
    ...
    hbar_df$var_type <- sapply(1:nrow(hbar_df), function(row)
            ifelse(length(grep(".Scaled", hbar_df[row, "variable"], fixed=TRUE)) == 0,
                            "Simplicity.Raw", "Simplicity.Scaled"))
    hbar_df$var_label <- 
        gsub("Simplicity.", "",
...
#                                     data.frame(facet=rep(" ", nrow(pn_df))))
    ...
    print(myplot_hbar(pn_df, ".rownames", paste0(metrics, ".Weighted")) + 
        scale_fill_discrete("Metrics", 
                            labels=gsub("Simplicity.", "", metrics, fixed=TRUE)))
